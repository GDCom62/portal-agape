...
# --- 4. ÁREA LOGADA ---
    ...
    if adm:
        ...
        t1, t2 = st.tabs(["Conteúdo", "Financeiro"])
        with t1:
    ...
    # TELA AGENDA
        ...
        for _, e in evs.iterrows():
            st.markdown(f'<div class="card-post"><b>{e["dia_semana"]} às {e["hora"]}</b><br>{e["titulo"]}</div>', unsafe_allow_html=True)
    ...
    # TELA OFERTAS
        ...
        st.markdown('<div class="card-post" style="text-align:center;"><h3>PIX: financeiro@agape.com</h3><p>Deus abençoe sua generosidade!</p></div>', unsafe_allow_html=True)
    ...
    # TELA CHAT
        ...
        st.markdown(f'''<div style="text-align:center; padding:40px; background:white; border-radius:15px; border:1px solid #ddd;">
            <a href="{link}" target="_blank" style="background:#1877f2; color:white; padding:15px 30px; text-decoration:none; border-radius:30px; font-weight:bold; display:inline-block;">ABRIR CHAT PREMIUM</a></div>''', unsafe_allow_html=True)
