...
# --- 4. ÁREA LOGADA ---
    ...
    with st.sidebar:
        ...
        menu = st.radio("Menu", ["📢 Mural", "📖 Bíblia", "🎥 Comunhão", "💰 Financeiro"])
    ...
    if menu == "🎥 Comunhão":
        ...
        with c1:
            ...
            dest = st.radio("Conversar com:", ["Todos (Grupo)"] + outros, key="chat_dest")
            
            st.divider()
            # VÍDEO RESOLVIDO - Usando link completo e domínio estável
            sala_id = f"Agape{limpar_nome(u['nome'])}{limpar_nome(dest)}"
            url_v = f"https://8x8.vc{sala_id}"
            
            st.write("📹 **Vídeo Chamada**")
            st.components.v1.html(f'<iframe src="{url_v}" allow="camera; microphone; fullscreen" style="height:380px; width:100%; border:3px solid #b8860b; border-radius:15px; background:black;"></iframe>', height=400)
        ...
        with c2:
            ...
            # Busca de mensagens garantida (Privado ou Grupo)
            if dest == "Todos (Grupo)":
            ...
            with chat:
                for _, r in df_msg.iterrows():
                    ...
                    if r['anexo_data'] and r['anexo_data'] != "":
                        st.download_button(label=f"📁 Baixar {r['anexo_nome']}", data=base64.b64decode(r['anexo_data']), file_name=r['anexo_nome'], key=f"dl_{r['id']}")
                ...
                if st.form_submit_button("Enviar"):
                    if txt or arq:
                        ...
                        executar_query("INSERT INTO mensagens (de_user, para_user, texto, anexo_nome, anexo_data, data) VALUES (:d,:p,:t,:an,:ad,:dt)", 
                                      {"d":u['nome'], "p":dest, "t":txt, "an":arq.name if arq else "", "ad":b64, "dt":datetime.now().strftime("%H:%M")})
                        st.rerun()
        ...
        if admin_mode:
            with st.form("f_mural", clear_on_submit=True):
                tit, cont, foto = st.text_input("Título"), st.text_area("Conteúdo"), st.file_uploader("Foto")
        ...
        for _, av in consultar_db("SELECT * FROM avisos ORDER BY id DESC").iterrows():
            st.markdown(f'<div class="card-mural"><h3>{av["titulo"]}</h3><p>{av["conteudo"]}</p></div>', unsafe_allow_html=True)
        ...
        if consultar_db("SELECT COUNT(*) as t FROM biblia").iloc[0]['t'] < 10 and os.path.exists("acf.json"):
            with st.spinner("Carregando..."):
                with open("acf.json", "r", encoding="utf-8-sig") as f:
                    dados = json.load(f)
                    for l in dados:
                        for n_c, cap in enumerate(l['chapters']):
                            for n_v, t in enumerate(cap):
                                executar_query("INSERT INTO biblia (livro, cap, ver, texto) VALUES (:l,:c,:v,:t)", {"l":l['name'],"c":n_c+1,"v":n_v+1,"t":str(t)})
            st.rerun()
        ...
        if not df.empty:
            st.metric("Saldo Abençoado", f"R$ {df[df['tipo']=='Entrada']['valor'].sum() - df[df['tipo']=='Saída']['valor'].sum():,.2f}")
