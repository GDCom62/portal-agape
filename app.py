...
try:
    ...
    st.error("Erro ao conectar ao Redis para o Chat.")
...
def init_db():
    ...
    # Carregamento Automático da Bíblia
        ...
        with st.form("cad"):
            ...
            if st.form_submit_button("Cadastrar"):
                try:
                    ...
                    st.success("Sucesso! Vá ao login."); st.session_state.tela = "login"; st.rerun()
...
# --- 4. ÁREA LOGADA ---
    ...
    r_db.set(f"online:{u['nome']}", "online", ex=60) # Heartbeat para o Chat
    aplicar_estilo()

    with st.sidebar:
        st.write(f"👤 **{u['nome']}**")
        menu = st.radio("Menu", ["🏠 Mural", "📅 Agenda", "📖 Bíblia", "🙏 Orações", "🤝 Ofertas", "💰 Financeiro", "💬 Chat Online"])
        adm = st.checkbox("⚙️ Administração") if u['is_admin'] else False
        if st.button("Sair"): 
    ...
    if adm:
        ...
        t1, t2 = st.tabs(["Mural e Agenda", "Financeiro"])
        with t1:
            with st.form("f_adm_m"):
        ...
        if not df_fin.empty:
            st.table(df_fin[['descricao', 'valor', 'tipo', 'data']])
            ativos = df_fin[df_fin['tipo'].str.contains("Ativo")]['valor'].sum()
            passivos = df_fin[df_fin['tipo'].str.contains("Passivo")]['valor'].sum()
            st.metric("Saldo Líquido", f"R$ {ativos - passivos:.2f}")

    elif menu == "💬 Chat Online":
        link = f"{URL_CHAT_RAILWAY}?user={u['nome']}"
        st.markdown(f'''<div style="text-align:center; padding:40px; background:white; border-radius:15px; border:1px solid #ddd;">
            <h2>Ágape Premium Chat</h2><a href="{link}" target="_blank" style="background:#1877f2; color:white !important; padding:15px 30px; text-decoration:none; border-radius:30px; font-weight:bold; display:inline-block;">ENTRAR NO CHAT</a></div>''', unsafe_allow_html=True)

    elif menu == "🙏 Orações":
        with st.form("f_or"):
            ped = st.text_area("Pedido de Oração")
            if st.form_submit_button("Enviar"):
                executar_query("INSERT INTO oracoes (nome, pedido, status, data) VALUES (:n,:p,'Pendente',:d)", {"n":u['nome'], "p":ped, "d":datetime.now().strftime("%d/%m")})
