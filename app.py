...
if not st.session_state.logado:
    ...
    if adm_mode:
        ...
        t1, t2, t3 = st.tabs(["Mural/Agenda", "Financeiro", "Sistema"])
        with t1:
            with st.form("f_m"):
        ...
        with t3:
            if st.button("📖 Importar Bíblia (acf.json)"):
        ...
        for _, e in consultar_db("SELECT * FROM eventos").iterrows():
        ...
        if livros.empty: st.warning("Aguardando Admin carregar a Bíblia.")
        else:
        ...
        st.markdown('<div class="card-post" style="text-align:center;"><h3>Contribua via PIX</h3><p>Chave: <b>financeiro@agape.com</b></p></div>', unsafe_allow_html=True)

    elif menu == "💰 Financeiro":
        st.title("Transparência Mensal")
        res_m = consultar_db("SELECT DISTINCT mes_ref FROM financeiro")
        if not res_m.empty:
            m_sel = st.selectbox("Mês", res_m['mes_ref'])
