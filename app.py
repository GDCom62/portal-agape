...
if not st.session_state.logado:
        ...
        with st.form("login"):
            ...
            if st.form_submit_button("Entrar"):
                ...
                if not res.empty and check_password_hash(res.iloc[0]['senha'], s):
                    ...
                    r_db.set(f"online:{res.iloc[0]['nome']}", "online", ex=60) # Registra presença
                    st.rerun()
                st.error("Erro no login.")
        st.button("Cadastre-se", on_click=lambda: st.session_state.update({"tela": "cadastro"}))
    else:
        with st.form("cad"):
    ...
    r_db.set(f"online:{u['nome']}", "online", ex=60) # Batimento cardíaco
    aplicar_estilo()
    with st.sidebar:
        st.write(f"👤 **{u['nome']}**")
