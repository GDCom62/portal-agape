...
# URL OFICIAL DO SEU CHAT NO RAILWAY
...
if not st.session_state.logado:
    ...
    if menu == "🏠 Feed":
        ...
        # (Seu código de feed continua aqui...)
        st.markdown("Bem-vindo ao mural!")

    # --- MENU: CHAT ONLINE (INTEGRAÇÃO) ---
    elif menu == "💬 Chat Online":
        st.title("💬 Chat Comunitário Ágape")
        
        # Link dinâmico: Passa o nome do usuário logado no portal para o Chat automaticamente
        link_final = f"{{URL_CHAT_RAILWAY}}?user={{u['nome']}}&room=Geral"
        
        st.info(f"Você está conectado como **{{u['nome']}}**")
        
        # Embutindo o Chat com permissões para Vídeo e Microfone
        st.markdown(f"""
